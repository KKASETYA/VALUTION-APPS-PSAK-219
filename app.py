...
# ==========================================
# 1. DATABASE OTOMATIS (YIELD CURVE & TMI IV)
# ==========================================
YIELD_CURVE_PHEI = {
...
def get_spot_rate(tenor):
    tenor_int = int(min(max(1, tenor), 30))
    # Cari tenor terdekat
    for t in sorted(YIELD_CURVE_PHEI.keys()):
        if tenor_int <= t:
            return YIELD_CURVE_PHEI[t]
    return 0.0637
...
# ==========================================
# 2. PARSER OTOMATIS DARI EXCEL KLIEN
# ==========================================
def parse_excel_automated(file_or_buffer, sheet_name=0):
    ...
    for idx in range(data_start_idx, len(df_raw)):
        ...
        gol = row.iloc[3] if len(row) > 3 else 'I' # Kolom Golongan jika ada
        dob = row.iloc[4] if len(row) > 4 else None
        doe = row.iloc[5] if len(row) > 5 else None
        salary = row.iloc[6] if len(row) > 6 else 0.0
        dplk = row.iloc[7] if len(row) > 7 else 0.0
        
        # Penyesuaian jika susunan kolom template standar (tanpa kolom gol terpisah)
        if isinstance(dob, (datetime.datetime, pd.Timestamp)) or (isinstance(dob, str) and '-' in dob):
            pass # Sesuai format standar
        else:
            # Fallback index jika kolom geser
            dob = row.iloc[3]
            doe = row.iloc[4]
            salary = row.iloc[5]
            dplk = row.iloc[6]
...
# ==========================================
# 3. ENGINE AKTUARIA OTOMATIS (YIELD CURVE + TMI IV)
# ==========================================
class AutomatedActuarialEngine:
    def __init__(self, salary_increase):
    ...
    def calculate_fully_automated(self, current_age, past_service, current_salary, ret_age):
        ...
        for t in range(years_to_retire):
            ...
            # Otomatis menggunakan suku bunga spot rate berdasarkan tenor tahun ke-t
            discount_t = get_spot_rate(t + 1)
            
            b_death = salary_t * ((2 * up_t) + upmk_t) * 1.15
            b_disab = salary_t * ((2 * up_t) + upmk_t) * 1.15
            v = 1 / ((1 + discount_t) ** (t + 1))
        ...
        discount_ret = get_spot_rate(years_to_retire)
        v_ret = 1 / ((1 + discount_ret) ** years_to_retire)
...
# ==========================================
# 4. GENERATOR LAPORAN PDF OTOMATIS
# ==========================================
...
def generate_automated_pdf(results_dict, dplk_dict, paid_dict, salary_inc, company_name, report_no):
    ...
    elements.append(Paragraph(f"<b>FULLY AUTOMATED ACTUARIAL VALUATION (PSAK 219)<br/>Valuation Period Ended December 31, {cur_yr}</b><br/><br/><b>FINAL REPORT NO. {report_no}</b>", sub_style))
    elements.append(PageBreak())
    
    elements.append(Paragraph("<b>I. Executive Summary & Automated Yield Curve Assumptions</b>", h_style))
    assumption_data = [
        ["Parameter Asumsi", "Metode / Nilai Otomatis"],
        ["Tingkat Diskonto", "Yield Curve Berjenjang Resmi (PHEI/IBPA)"],
        ["Tingkat Kenaikan Gaji", f"{salary_inc*100:.2f}% per tahun"],
        ["Tabel Mortalita", "TMI IV (Otomatis per Usia Individu)"],
        ["Metode Perhitungan", "Projected Unit Credit (PUC) Full Automated"]
    ]
...
# ==========================================
# 5. STREAMLIT WEB INTERFACE (FULLY AUTOMATED)
# ==========================================
st.set_page_config(page_title="Valuasi Aktuaria Fully Automated", layout="wide")
st.title("📄 Generator Laporan Aktuaria Otomatis Penuh (Yield Curve & TMI IV)")
    ...
    try:
        ...
        for sh in xl_file.sheet_names:
            if 'asumsi' in sh.lower():
                continue
            detected_yr, df_emp, total_paid = parse_excel_automated(uploaded_file, sheet_name=sh)
            datasets_to_process[detected_yr] = df_emp
            benefit_paid_dict[detected_yr] = total_paid
        st.success(f"Berhasil membaca sheet secara otomatis: {list(datasets_to_process.keys())}")
    except Exception as e:
        st.error(f"Gagal membaca file: {e}")

st.markdown("---")
if st.button("Jalankan Valuasi Otomatis Penuh 🚀") and datasets_to_process:
    with st.spinner("Sistem sedang menghitung menggunakan Yield Curve dan TMI IV otomatis..."):
        results_dict = {}
        dplk_dict = {}
        ...
        for key, df_input in datasets_to_process.items():
            ...
            for _, row in df_input.iterrows():
                ...
                # Otomatis menentukan usia pensiun berdasarkan golongan
                ret_age = 56 if any(g in gol.upper() for g in ['IV', 'V', 'VI', '4', '5', '6']) else 55
                
                engine = AutomatedActuarialEngine(asumsi_gaji)
                kalkulasi = engine.calculate_fully_automated(current_age, past_service, salary, ret_age)
        ...
        st.success("Perhitungan Otomatis Selesai!")

if st.session_state.get("calculated"):
    st.subheader("📊 Hasil Kalkulasi Otomatis")
    ...
    pdf_file = generate_automated_pdf(
        res_dict, dp_dict, pd_dict, asumsi_gaji, input_perusahaan, nomor_laporan
    )
    
    st.download_button(
        label="📥 Download Laporan PDF Otomatis Penuh",
        data=pdf_file,
        file_name=f"AUTOMATED_REPORT_{input_perusahaan.replace(' ', '_')}.pdf",
