...
# ==========================================
# 6. STREAMLIT INTERFACE (BERANDA & KALKULATOR)
# ==========================================
st.set_page_config(page_title="Portal Aktuaria Setya Gunawan", layout="wide")

# Sidebar Navigasi Utama
st.sidebar.title("📌 Menu Navigasi")
menu = st.sidebar.radio("Pilih Halaman:", ["🏠 Beranda (Home)", "🧮 Kalkulator Valuasi Aktuaria"])

if menu == "🏠 Beranda (Home)":
    st.title("🌟 Selamat Datang di Portal Mandiri Valuasi Aktuaria")
    st.markdown("### Kantor Konsultan Aktuaria Setya Gunawan")
    st.write("Platform web profesional berstandar **PSAK 219** yang dilengkapi dengan pencocokan kurva *yield zero-coupon* resmi **PHEI IGSYC**[cite: 1] serta generator laporan audit aktuaria lengkap secara instan.")
    
    st.markdown("---")
    col1, col2, col3 = st.columns(3)
    with col1:
        st.info("### 📈 Akurat & Sesuai SBN\nMenggunakan referensi kurva *yield* harian resmi PHEI untuk ketepatan diskonto liabilitas[cite: 1].")
    with col2:
        st.success("### ⚡ Multi-Tahun & Fleksibel\nMendukung data historis dari tahun **2021 hingga 2026** melalui Excel atau editor interaktif[cite: 1].")
    with col3:
        st.warning("### 📄 Laporan PDF Resmi\nLangsung unduh laporan formal dwibahasa lengkap dengan neraca, OCI, dan rekonsiliasi[cite: 1].")
        
    st.markdown("---")
    st.subheader("💡 Panduan Penggunaan Singkat:")
    st.write("1. Buka menu **Kalkulator Valuasi Aktuaria** di sidebar sebelah kiri.")
    st.write("2. Masukkan parameter perusahaan, asumsi kenaikan gaji, dan usia pensiun.")
    st.write("3. Pilih metode masukan data (Unggah file Excel multi-tahun atau ketik langsung di tabel editor web)[cite: 1].")
    st.write("4. Klik **Jalankan Valuasi Otomatis** untuk melihat ringkasan dan mengunduh laporan PDF resmi.")

elif menu == "🧮 Kalkulator Valuasi Aktuaria":
    st.title("📄 Generator Laporan Aktuaria (PHEI IGSYC Yield Matching)")
    ...
    st.sidebar.info("💡 **Standar Aktuaris:** Suku bunga diskonto ditentukan otomatis lewat *yield curve matching* PHEI IGSYC[cite: 1].")
    ...
    if metode_input == "Upload File Excel Multi-Tahun":
        ...
        st.info("Masukkan data karyawan langsung per tahun menggunakan tabel interaktif di bawah (Rentang 2021 - 2026)[cite: 1].")
